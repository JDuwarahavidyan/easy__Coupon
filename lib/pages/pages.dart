export './login/get_started.dart';
export './login/login_page.dart';
export 'login/pw_email_reset_page.dart';
export './login/splash_screen.dart';
export './student/student_home.dart';
export 'canteen/canteen_a/qr_generation.dart';
export './canteen/canteen_b/canteen_b.dart';
export 'student/settings_page.dart';
export 'student/qr_scanning.dart';
export 'student/student_report.dart';
export 'login/register_screen.dart';
export 'login/new_user_pw_reset_page.dart';



